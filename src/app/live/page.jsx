...
export default function LivePage() {
  ...
  const handlePlaceBet = useCallback(async (betType, betDriver, betAmount, betMultiplier) => {
    if (!user) {
  ...
  const handleLoginSuccess = useCallback(() => {
    if (pendingBet) {
      handlePlaceBet(pendingBet.betType, pendingBet.betDriver, pendingBet.betAmount, pendingBet.betMultiplier)
      setPendingBet(null)
    }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            {isRaceFinished ? (
              <Button onClick={() => setShowFinalDashboard(true)} className="w-full bg-primary hover:bg-primary/90 text-white">
                View Race Dashboard
              </Button>
            ) : (
              <Button onClick={() => setShowRaceDetails(true)} className="w-full bg-primary hover:bg-primary/90 text-white">
